...
object MonadOps {
  ...
  // 6 points.
  def mapFuture[X](xfs: Seq[Future[X]])(implicit executor: ExecutionContext): Seq[Future[Either[Throwable, X]]] = for(x <- xfs) yield sequence(x)// TO BE IMPLEMENTED

  /**
   * Sequence the Seq of xeTry of X into a Try of Seq of X such that if any of the input elements is a Failure,
